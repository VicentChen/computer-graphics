...
int main(int argc, char const *argv[]) {
  ...
  Shader cube_shader("materials-HW1.vs", "materials-HW1.fs");
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    cube_shader.setVec3("material.diffuse", 0.0f, 0.51f, 0.51f);
    cube_shader.setVec3("material.specular", 0.5f, 0.5f, 0.50f);
    cube_shader.setFloat("material.shininess", 32.0f);

    vec3 light_color;
    light_color.x = 1.0f;//sin(glfwGetTime() * 2.0f);
    light_color.y = 1.0f;//sin(glfwGetTime() * 0.7f);
    light_color.z = 1.0f;//sin(glfwGetTime() * 1.3f);

    vec3 diffuse_color = light_color * vec3(0.5f); // decrease the influence
    vec3 ambient_color = diffuse_color * vec3(0.2f); // low influence

    cube_shader.setVec3("light.position", light_pos);
    cube_shader.setVec3("light.ambient", 1.0f, 1.0f, 1.0f);
    cube_shader.setVec3("light.diffuse", 1.0f, 1.0f, 1.0f); // darken the light a bit to fit the scene
